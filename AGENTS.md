...
- **Backend**: FastAPI app in `app/` exposing task and activity endpoints.
- **Storage**: a local JSON file (`data/tasks.json`), loaded into an in-memory dict and rewritten on every mutation (`app/storage.py`). No database.
...
```
Then browse to `http://127.0.0.1:5500`. CORS in `app/main.py` only allows origin `http://localhost:5500` (not `127.0.0.1:5500`).

**Run tests** (from repo root, venv active):
```powershell
pytest -v
```
`tests/conftest.py` adds the repo root to `sys.path`. README states 24 tests currently pass.

- `tests/verify_a.py` **is** a supported entry point as of the final-project restructure (`python tests/verify_a.py`, imports `app.models` directly) — it previously raised `ModuleNotFoundError` when `app` lived under a now-removed `backend/` directory; that's fixed now that `app/` is at the repo root.
...
- **Task status** (`app/models.py`, `TaskStatus`): `ToDo`, `InProgress`, `Done`.
...
- **Tags** (`app/tags.py`): at most 10 tags, each ≤30 characters, non-blank after trimming; must be a list of strings.
- **Strict request bodies**: `TaskCreate`/`TaskUpdate` use `extra="forbid"` — unknown fields return `422`.
- **Partial-update (`PATCH`) quirks** (`app/storage.py`):
...
- **Activity events** (`storage.py`): `create`, `update`, `status_change`, `delete` — recorded on every mutation and persisted alongside tasks in `data/tasks.json`.
...
- **No `app/` changes without explicit approval**: `app/main.py`, `models.py`, `storage.py`, `business_rules.py`, and `tags.py` implement the graded business logic for this module. Do not modify any file under `app/` (or `frontend/index.html`, `requirements.txt`, `Dockerfile`) unless the user has explicitly approved that specific change in the current thread. Documentation files (`README.md`, `docs/**`, `AGENTS.md`) are outside this restriction unless the user says otherwise.
...
- **No destructive commands** — no deleting/overwriting `data/tasks.json`, force-pushing, `git reset --hard`, `rm -rf`, or similar, without explicit user confirmation for that specific action.
- **Cite files, don't paraphrase from memory.** Reference the exact file and, where useful, line numbers (e.g. `app/business_rules.py:6-10`) instead of restating rules generically.
